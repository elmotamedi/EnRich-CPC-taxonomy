...
class BERTClass(torch.nn.Module):
    def __init__(self, num_labels):
        ...
        self.bert_model = AutoModel.from_pretrained('sentence-transformers/all-MiniLM-L6-v2', return_dict=True)
        self.config = AutoConfig.from_pretrained('sentence-transformers/all-MiniLM-L6-v2', return_dict=True)
        self.dropout = torch.nn.Dropout(0.3)
        self.linear = torch.nn.Linear(384, num_labels)
...
def main(train_file, val_file, test_file, output_dir, model_name, label, output_stat_path):
    ...
    print(f"number of labels are : {num_labels}")
            ...
            if step == len(train_dataloader) - 1:
                ...
                if highest_valid_acc == 0 or avg_val_accuracy > highest_valid_acc:
                    ...
                    if accelerator.is_main_process:
                        ...
                        torch.save(unwrapped_model.state_dict(), os.path.join(PATH, 'pytorch_model.bin'))
                        # Save the tokenizer
...
if __name__ == "__main__":
    ...
    output_dir = 'D:/Elham/EnrichMyData/EnrichData_PC/model/sbert_model/999/label'
    output_stat_path = "D:/Elham/EnrichMyData/EnrichData_PC/model/sbert_model/999/training_stats.json"
